...
# VERSION 3: Added "Summoning Sickness" (minions can't attack the turn they are summoned).
...
class Minion:
    ...
    age: int = 0
    attack: int = 0
    can_attack: bool = True  # <-- NEW: For summoning sickness
    ...
    def start_of_owner_turn(self):
        ...
        """
        self.age += 1
        self.can_attack = True  # <-- NEW: Sickness wears off
    ...
    def __str__(self):
        # NEW: Show summoning sickness status
        status = ""
        if not self.can_attack:
            status = " (Sick)"
            
        if self.name == 'Specter':
            return f"👻 Specter (HP:{self.hp}, ATK:{self.attack}, Age:{self.age}){status}"
        if self.name == 'Bubble':
            # Bubbles don't attack, but status is harmless
            return f"🧊 Bubble (HP:{self.hp}, Age:{self.age}/4){status}" 
        return f"{self.name}(HP:{self.hp}, Age:{self.age}){status}"
...
class Player:
    ...
    def start_turn(self):
        ...
        1. Shield decay
        2. Burn damage
        3. Minion start-of-turn triggers (Bubble MP/Explode, Sickness)
        """
        msgs = []
        # 1. Shield decay
        if self.shield > 0:
...
def perform_summon(attacker: Player):
    ...
    if choice == 1:
        ...
        if len(attacker.minions) < 2:
            attacker.mp -= cost
            # NEW: Set can_attack=False on summon
            b = Minion(name='Bubble', hp=6, owner=attacker, can_attack=False)
        ...
        if len(attacker.minions) < 2:
            attacker.mp -= cost
            # NEW: Set can_attack=False on summon
            s = Minion(name='Specter', hp=10, owner=attacker, attack=1, special={'burn_on_hit':1}, can_attack=False)
...
def minion_attack_phase(player: Player, enemy: Player):
    ...
    NEW: Only minions with can_attack=True will attack.
    """
    # NEW: Filter for m.can_attack
    specters = [m for m in list(player.minions) if m.name == "Specter" and m.can_attack and m in player.minions]
